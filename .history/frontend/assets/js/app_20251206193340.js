...
// UrbanStep Shoe Shop SPA Routingconst routes = {
...
async function navigate(route) {
  ...
  if (route === 'products') attachAddToCartHandlers(); // 🔹 ključno za “Add to Cart”
}
...
//  “Add to Cart” logic (SPA-friendly)
function attachAddToCartHandlers() {
  ...
  addToCartButtons.forEach(button => {
    button.addEventListener("click", (e) => {
      ...
      // Store in localStorage (simulate cart/orders)
      ...
      setTimeout(() => {
        // Navigate to Orders page (SPA style)
        window.history.pushState({ route: 'orders' }, '', '#orders');
        navigate('orders');
      }, 800);
    });
  });
